...
semantics = grammar.createSemantics().addOperation('ast', () => {
    ...
    MatchExp = (idExp, matchArray) => {return new MatchExpression(idExp.tree(), matchArray.tree());},
    Param = () => {},
    Exp_reg = (left, op, right) => {return new BinaryExpression(left.tree(), op.sourceString, right.tree());},
    Exp_pass = () => {},
    BoolAndExp_reg = (left, op, right) => {return new BinaryExpression(left.tree(), op.sourceString, right.tree());},
    BoolAndExp_pass = () => {},
    RelExp_reg = (left, op, right) => {return new BinaryExpression(left.tree(), op.sourceString, right.tree());},
    RelExp_pass = () => {},
    AddExp_reg = (left, op, right) => {return new BinaryExpression(left.tree(), op.sourceString, right.tree());},
    AddExp_pass = () => {},
    MulExp_reg = (left, op, right) => {return new BinaryExpression(left.tree(), op.sourceString, right.tree());},
    MulExp_pass = () => {},
    ExponExp_reg = (base, exponent) => {return new BinaryExpression(base.tree(), "^", exponent.tree());},
    ExponExp_pass = () => {},
    PrefixExp = () => {},
    ParenExp_reg = (left, exp, right) => {return new ParenthesisExpression(exp.tree());}
